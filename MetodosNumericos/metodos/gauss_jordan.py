...
def gauss_jordan(A, b):
    ...
    for i in range(n):
        # Pivoteo: se busca la fila con el mayor valor absoluto en la columna actual para mejorar la estabilidad numérica
        ...
        #steps.append(aug.copy())                    # Se podría guardar este paso si queremos visualizarlo
        
        # Escalonar: normalizamos la fila del pivote dividiéndola por el valor del pivote
        ...
        for j in range(n):
            if j != i:                      # Aseguramos que no afectamos la fila del pivote
                ...
                aug[j] -= factor * aug[i]   # Restamos la fila escalada para hacer ceros
                steps.append(aug.copy())    # Guardamos el estado de la matriz después de cada eliminación
